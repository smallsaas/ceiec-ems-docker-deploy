...
if [ $running = "produce" ];then
   ...
   do
      ...
      ## start each endpoint
      if [ ${GREENFIELD} ];then
         echo "java -jar $ep --spring.profiles.active=produce"
         java -jar $ep --spring.profiles.active=produce  
      else
         ...
         echo "java $JAVA_OPTIONS -jar $ep --spring.profiles.active=produce --server.port=8080"
         java $JAVA_OPTIONS -jar $ep --spring.profiles.active=produce --server.port=8080
      fi
